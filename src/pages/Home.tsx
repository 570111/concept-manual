...
function ContinueLearningCard({ concepts }: { concepts: { id: string; icon: string; title: string }[] }) {
  ...
  if (doneCount === 0) {
    return (
      <div className="mx-auto flex max-w-xl items-center justify-between gap-4 rounded-2xl border border-emerald-500/20 bg-slate-900/60 p-4">
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        {modules.map((m) => (
          <Link
            key={m.to}
            to={m.to}
            className="group tech-card p-6 transition hover:-translate-y-1 hover:border-emerald-500/30 hover:shadow-[0_0_36px_-10px_rgba(16,185,129,0.5)]"
          >
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {categoryOrder.map((cat) => (
            <div key={cat} className="tech-card p-5">
              ...
              <p className="mt-2 text-sm text-slate-400">{categoryInfo[cat].desc}</p>
