  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="flex items-start gap-4">
        <div className="flex h-14 w-14 flex-none items-center justify-center rounded-2xl bg-emerald-500/10 text-3xl ring-1 ring-emerald-500/20">
          {concept.icon}
        </div>
        <div>
          <h1 className="text-2xl font-extrabold text-white">{concept.title}</h1>
          <p className="mt-0.5 text-sm text-slate-500">{concept.aka}</p>
        </div>
      </div>
      ...
      <section className="space-y-3">
        ...
        <p className="leading-relaxed text-slate-300">{content.explain}</p>
      </section>
          ...
          {content.apply.map((tip, i) => (
            <div key={i} className="tech-card p-4">
              <div className="flex items-start gap-2.5">
                <span className="flex h-6 w-6 flex-none items-center justify-center rounded-full bg-emerald-500/15 text-xs font-bold text-emerald-300 ring-1 ring-emerald-400/40">
                  {i + 1}
                </span>
                <div>
      ...
      {content.misconceptions.length > 0 && (
        <section className="space-y-2">
          <h2 className="text-sm font-bold text-slate-500">常见误解</h2>
          <ul className="space-y-2">
            {content.misconceptions.map((m, i) => (
              <li
                key={i}
                className="flex gap-2 rounded-2xl border border-slate-800 bg-slate-900/60 p-3.5 text-sm leading-relaxed text-slate-300"
              >
      ...
      <section className="tech-card p-4">
        <div className="flex items-center gap-2">
          <span className="rounded-full border border-slate-700 bg-slate-800/60 px-2.5 py-0.5 text-xs font-bold text-slate-400">
      ...
      {content.related.length > 0 && (
        <section className="space-y-2">
          <h2 className="text-sm font-bold text-slate-500">关联概念</h2>
      ...
      {questions.length > 0 && (
        <section className="space-y-3 border-t border-slate-800 pt-6">
          {!quizOpen ? (
            <div className="flex items-center justify-between gap-4 rounded-2xl border border-sky-500/25 bg-sky-500/5 p-5 shadow-[0_0_24px_-14px_rgba(56,189,248,0.6)]">
              <div>
      ...
      <div className="flex items-center justify-between border-t border-slate-800 pt-6 text-sm">
        {prev ? (
          <Link to={`/concepts/${prev.id}`} className="font-medium text-slate-400 hover:text-emerald-400">
            ← {prev.title}
          </Link>
        ) : (
          <span />
        ...
        {next ? (
          <Link to={`/concepts/${next.id}`} className="font-medium text-slate-400 hover:text-emerald-400">
            {next.title} →
          </Link>
        ) : (
          <span />
