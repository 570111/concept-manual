-- 破局手册：给 concept_content 表新增 real_case / misconceptions 两列，并更新读取函数。
-- 在 Supabase 控制台 -> SQL Editor 里粘贴整个文件，点 Run 执行一次即可。
-- 不会影响已有的密钥、概念、测验数据。

alter table concept_content add column if not exists real_case jsonb not null default '{"title":"","body":""}'::jsonb;
alter table concept_content add column if not exists misconceptions jsonb not null default '[]'::jsonb;
